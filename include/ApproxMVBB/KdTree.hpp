...
public:
    ...
    */
    ...
    /** Move from node */
    Node(Node&& n) : Base(std::move(n)), m_bound(n.m_bound), m_data(n.m_data)
    {
        n.m_data = nullptr;
    }
...
public:
    ...
    TreeBase(TreeBase&& t) : m_nodes(std::move(t.m_nodes)), m_leafs(std::move(t.m_leafs)), m_root(t.m_root)
    {
...
public:
    ...
    /** Move constructor */
    template <typename Traits>
    TreeSimple(TreeSimple<Traits>&& tree) : Base(std::move(tree)), m_statistics(std::move(tree.m_statistics))
    {
    }
    /** Copy the tree */
    template <typename Traits>
    TreeSimple(const TreeSimple<Traits>& tree) : Base(tree), m_statistics(tree.m_statistics)
    {
    }
